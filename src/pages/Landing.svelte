  ...
  <section class="stats-section page-wrapper">
    ...
    <div class="bento-grid">
      ...
      <BentoCard span={3}>
        ...
        {#if !loading}
          <p class="text-caption">MCA · {displayMca} &nbsp;|&nbsp; MSc CS · {displayMsc}</p>
        {/if}
      </BentoCard>
      ...
      <BentoCard span={6} rows={2}>
        ...
        {:else}
          <div class="batch-split">
            <div class="batch-bar">
              <div class="batch-fill mca" style="width:{totalStudents ? (aggregates.mcaCount/totalStudents*100) : 50}%"></div>
              <div class="batch-fill msc" style="width:{totalStudents ? (aggregates.mscCount/totalStudents*100) : 50}%"></div>
            </div>
            <div class="batch-legend">
              <span class="badge badge-mca">MCA — {aggregates.mcaCount || 0}</span>
              <span class="badge badge-msc">MSc CS — {aggregates.mscCount || 0}</span>
            </div>
          </div>
        {/if}
      </BentoCard>
      ...
      <BentoCard span={6} rows={3}>
        ...
        {:else}
          ...
          {#if skills.length > 8}
            <a href="/#/directory" class="text-caption" style="margin-top:0.75rem;display:block">
...
<style>
  ...
  /* Decorative circles */
  .hero-deco { position: absolute; right: -60px; top: -60px; pointer-events: none; z-index: 1; }
  .deco-circle { position: absolute; border-radius: 50%; opacity: 0.12; }
  .c1 { width: 320px; height: 320px; background: var(--accent); top: 0; right: 0; }
  .c2 { width: 200px; height: 200px; background: var(--chart-2); top: 60px; right: 80px; }
  .c3 { width: 120px; height: 120px; background: var(--chart-3); top: -30px; right: 200px; }

  .stats-section, .search-section { padding-top: 3rem; }
  .stats-heading { margin-bottom: 1.25rem; }

  /* Batch split bar */
  .batch-split { display: flex; flex-direction: column; gap: 1rem; margin-top: 0.5rem; }
  .batch-bar { display: flex; height: 40px; border-radius: 0.5rem; overflow: hidden; background: var(--surface-border); }
  .batch-fill { height: 100%; transition: width 0.6s ease; }
  .batch-fill.mca { background: var(--chart-1); }
  .batch-fill.msc { background: var(--chart-2); }
  .batch-legend { display: flex; gap: 0.75rem; }
  ...
  .search-row { display: flex; gap: 0.75rem; justify-content: center; flex-wrap: wrap; }
  .search-input {
    background: var(--bg);
